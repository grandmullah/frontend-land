...
import RegisteredLands from './registeredlands'
...
export default class Registerland extends React.Component {
  ...
  handleSubmit (event) {
    ...
    const dy = new Date()
    this.setState({date:dy})
    const contract = this.props.drizzle.contracts.Lands
    const stackId = contract.methods['registerLand'].cacheSend(this.state.landnumber, this.state.size, this.state.location,this.state.log, this.state.lat, this.state.date, this.state.size,{
    ...
    return (
      <div>
        <Container className='gov'>
          <Row>
            <Col>
              <RegisteredLands drizzle={ this.props.drizzle } drizzleState={ this.props.drizzleState }/>
            </Col>
            <Col >
              <div>
                <h2> Register Land </h2>
